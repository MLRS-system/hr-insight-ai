...
from pptx import Presentation  # pptx 라이브러리 임포트
...
def get_content_from_input(path_or_url: str):
    """
    URL 또는 서버에 업로드된 파일(docx, pptx 포함) 경로를 읽어 콘텐츠와 타입을 반환합니다.
    """
    ...
    try:
        ...
        if file_extension == '.csv':
            ...
            print(f"    [WARNING] Unsupported file type: {file_extension}. Attempting to read as text.")
            try:
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    return f.read(), "text"
            except Exception as read_error:
                print(f"    [ERROR] Could not read unsupported file as text: {read_error}")
                return None, None
